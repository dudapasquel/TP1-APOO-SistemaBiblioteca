...
class SistemaBibliotecaCompleto:
    ...
    def criar_tela_login(self):
        ...
        cores = {"Bibliotecario": "#2B5CE6", "Aluno": "#28A745", "Professor": "#DC3545"}
        ...
        ctk.CTkLabel(login_frame, text="📧 Email:", font=ctk.CTkFont(size=16, weight="bold")).pack(pady=(30, 5))
        ...
        ctk.CTkLabel(login_frame, text="🔒 Senha:", font=ctk.CTkFont(size=16, weight="bold")).pack(pady=5)
        ...
        if BACKEND_DISPONIVEL:
            ...
            if self.tipo_usuario_selecionado in emails:
                ...
                threading.Timer(0.5, lambda: self.email_entry.insert(0, emails[self.tipo_usuario_selecionado])).start()
    ...
    def criar_aba_livros_completa(self, parent):
        ...
        headers = ["ID", "Título", "Autor", "ISBN", "Gênero", "Status", "Data Cadastro"]
        ...
        self.livros_content_frame = ctk.CTkScrollableFrame(list_frame, height=300)
        self.livros_content_frame.pack(fill="both", expand=True, padx=5, pady=5)
    ...
    def carregar_livros_completo(self):
        ...
        if BACKEND_DISPONIVEL:
            ...
            self.livros_atuais = Livro.listar_todos(limite=1000, incluir_inativos=self.incluir_inativos)
        else:
        ...
        ativos = len([l for l in self.livros_atuais if getattr(l, 'ativo', True)])
    ...
    def criar_linha_livro(self, livro: Livro, index: int):
        ...
        row_frame.bind("<Button-1>", lambda e, l=livro, f=row_frame: self.selecionar_livro_visual(l, f))
        
    
        id_label = ctk.CTkLabel(row_frame, text=str(getattr(livro, 'id', 'N/A')))
        id_label.grid(row=0, column=0, padx=5, pady=8)
        id_label.bind("<Button-1>", lambda e, l=livro, f=row_frame: self.selecionar_livro_visual(l, f))
        
    
        titulo_texto = livro.nome[:35] + "..." if len(livro.nome) > 35 else livro.nome
        titulo_label = ctk.CTkLabel(row_frame, text=titulo_texto, font=ctk.CTkFont(weight="bold"))
        titulo_label.grid(row=0, column=1, padx=5, pady=8, sticky="w")
        titulo_label.bind("<Button-1>", lambda e, l=livro, f=row_frame: self.selecionar_livro_visual(l, f))
        
    
        autor_label = ctk.CTkLabel(row_frame, text=livro.autor)
        autor_label.grid(row=0, column=2, padx=5, pady=8)
        autor_label.bind("<Button-1>", lambda e, l=livro, f=row_frame: self.selecionar_livro_visual(l, f))
        
    
        isbn_label = ctk.CTkLabel(row_frame, text=livro.isbn)
        isbn_label.grid(row=0, column=3, padx=5, pady=8)
        isbn_label.bind("<Button-1>", lambda e, l=livro, f=row_frame: self.selecionar_livro_visual(l, f))
        
    
        genero_label = ctk.CTkLabel(row_frame, text=livro.genero)
        genero_label.grid(row=0, column=4, padx=5, pady=8)
        genero_label.bind("<Button-1>", lambda e, l=livro, f=row_frame: self.selecionar_livro_visual(l, f))
        
    
        status_label = ctk.CTkLabel(row_frame, text=status_texto, text_color=cor_status, font=ctk.CTkFont(weight="bold"))
        status_label.grid(row=0, column=5, padx=5, pady=8)
        status_label.bind("<Button-1>", lambda e, l=livro, f=row_frame: self.selecionar_livro_visual(l, f))
        ...
        data_label.bind("<Button-1>", lambda e, l=livro, f=row_frame: self.selecionar_livro_visual(l, f))
    ...
    def selecionar_livro_visual(self, livro: Livro, row_frame):
        ...
        try:
            ...
            if hasattr(self, 'linha_selecionada') and self.linha_selecionada:
                try:
                    self.linha_selecionada.configure(fg_color=["gray92", "gray14"])
                except Exception:
                    pass 
    ...
    def mostrar_detalhes_livro(self):
        ...
        if not self.livro_selecionado:
            messagebox.showwarning("Atenção", "Por favor, selecione um livro primeiro!")
            return
    ...
    def janela_livro(self, modo: str, livro: Optional[Livro] = None):
        ...
        janela.title(f"{'Adicionar' if modo == 'adicionar' else 'Editar'} Livro")
        ...
        ctk.CTkLabel(campos_frame, text="📖 Título:", font=ctk.CTkFont(size=14, weight="bold")).pack(anchor="w", pady=(10, 5))
        nome_entry = ctk.CTkEntry(campos_frame, width=400, height=35, font=ctk.CTkFont(size=12))
        nome_entry.pack(pady=(0, 10))
        if livro:
            nome_entry.insert(0, livro.nome)
        
    
        ctk.CTkLabel(campos_frame, text="✍️ Autor:", font=ctk.CTkFont(size=14, weight="bold")).pack(anchor="w", pady=(10, 5))
        autor_entry = ctk.CTkEntry(campos_frame, width=400, height=35, font=ctk.CTkFont(size=12))
        autor_entry.pack(pady=(0, 10))
        if livro:
            autor_entry.insert(0, livro.autor)
        
    
        ctk.CTkLabel(campos_frame, text="🔢 ISBN:", font=ctk.CTkFont(size=14, weight="bold")).pack(anchor="w", pady=(10, 5))
        isbn_entry = ctk.CTkEntry(campos_frame, width=400, height=35, font=ctk.CTkFont(size=12))
        isbn_entry.pack(pady=(0, 10))
        if livro:
            isbn_entry.insert(0, livro.isbn)
        
    
        ctk.CTkLabel(campos_frame, text="🎭 Gênero:", font=ctk.CTkFont(size=14, weight="bold")).pack(anchor="w", pady=(10, 5))
        genero_combo = ctk.CTkComboBox(
            campos_frame,
            values=["Ficção", "Não-ficção", "Romance", "Mistério", "Fantasia", "Biografia", "História", "Ciência", "Tecnologia", "Outro"],
            width=400,
            height=35,
            font=ctk.CTkFont(size=12)
        )
        ...
        def salvar_livro():
            ...
            if BACKEND_DISPONIVEL:
                try:
                    if modo == "adicionar":
                        novo_livro = Livro(nome=nome, autor=autor, isbn=isbn, genero=genero)
                ...
                messagebox.showinfo("Simulação", f"Livro {'adicionado' if modo == 'adicionar' else 'editado'} com sucesso (modo simulação)")
                janela.destroy()
    ...
    def inativar_livro(self):
        ...
        if resposta:
            if BACKEND_DISPONIVEL:
                ...
                messagebox.showinfo("Operação Realizada", "Livro inativado com sucesso")
    ...
    def reativar_livro(self):
        ...
        if resposta:
            if BACKEND_DISPONIVEL:
                ...
                messagebox.showinfo("Operação Realizada", "Livro reativado com sucesso")
    ...
    def buscar_por_isbn(self):
        ...
        isbn = simpledialog.askstring("Buscar por ISBN", "Digite o ISBN do livro:")
            ...
            if BACKEND_DISPONIVEL:
                ...
                if livro:
                    ...
                    for widget in self.livros_content_frame.winfo_children():
                        ...
                        pass
                    messagebox.showinfo("Livro Encontrado", f"Livro '{livro.nome}' localizado na lista!")
                else:
                    messagebox.showinfo("Não Encontrado", f"Nenhum livro encontrado com ISBN: {isbn}")
            else:
                messagebox.showinfo("Busca Realizada", f"Busca por ISBN: {isbn}")
    ...
    def criar_aba_usuarios(self, parent):
        ...
        if BACKEND_DISPONIVEL:
            ...
            for usuario in usuarios:
                ...
                ctk.CTkLabel(user_frame, text=info_text, font=ctk.CTkFont(size=12)).pack(side="left", padx=10, pady=10)
        else:
            info_label = ctk.CTkLabel(parent, text="Funcionalidade disponível apenas com backend conectado")
    ...
    def tela_usuario_comum(self, titulo: str, tipo: str):
        ...
        if BACKEND_DISPONIVEL:
            ...
            for livro in livros:
                ...
                ctk.CTkLabel(livro_frame, text=livro_info, font=ctk.CTkFont(size=12)).pack(side="left", padx=10, pady=10)
                
                if tipo == "professor":
                    ctk.CTkButton(
                        livro_frame,
                        text="📋 Reservar",
                        command=lambda: messagebox.showinfo("Reserva", "Funcionalidade em desenvolvimento"),
                        width=100
                    ...
                    ctk.CTkButton(
                        livro_frame,
                        text="📖 Solicitar",
                        command=lambda: messagebox.showinfo("Empréstimo", "Funcionalidade em desenvolvimento"),
                        width=100
                    ).pack(side="right", padx=10, pady=5)
        else:
            ctk.CTkLabel(content_frame, text="📊 Dados disponíveis apenas com backend conectado").pack(pady=50)
    
    def fazer_logout(self):
        """Realiza o logout do usuário"""
        messagebox.showinfo("Logout", f"Logout realizado com sucesso! Até logo, {self.usuario_logado.nome}!")
